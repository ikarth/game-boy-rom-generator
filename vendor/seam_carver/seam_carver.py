...
from vendor.seam_carver.energy_functions import (
    simple_energy,
    dual_gradient_energy,
)
from vendor.seam_carver.utils import (
    display_energy_map,
    every_n,
    get_img_arr,
    highlight_seam,
...
def save_image_with_options(img, highlight, pad, seam, rotated, savename, original_height,
    ...
    img.save(os.path.dirname(savename)+'/'+os.path.basename(base)+'_'+str(point).zfill(len(str(savepoints[-1])))+'.'+ext)
